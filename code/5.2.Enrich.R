...
# 读取差异基因
degs_list <- read.csv("./res/5.Biology/table/degs.fc1.csv")
...
# save
outfile_tmp <- paste0(res_home,proj_name, "/table/", "GO.","fc1",".csv")
write.csv(kk.df, file=outfile_tmp)

### plot
outfile_tmp <- paste0(res_home, proj_name,"/pdf/","GO.","fc1", ".pdf")
...
kk.df <- as.data.frame(kk)
# save
outfile_tmp <- paste0(res_home,proj_name, "/table/", "KEGG.","fc1",".csv");outfile_tmp
write.csv(KEGG_res.df,file = outfile_tmp)

### plot
outfile_tmp <- paste0(res_home, proj_name,"/pdf/","KEGG.","fc1", ".pdf")
pdf(file = outfile_tmp, width = 8, height = 6)
...
# save
outfile_tmp <- paste0(res_home, proj_name,"/table/","Hallmark.","fc1",".csv");outfile_tmp
write.csv(kk.df, file = outfile_tmp)
